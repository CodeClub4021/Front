...
const AddGym = ({username}) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post(import.meta.env.VITE_BASE_URL + "gym", {
                ...formData,
                manager: username
            });
    ...
    return (
        <div>
            <div className="w-full flex justify-center py-6 px-5">
                <button
                    className="capitalize btn btn-accent border-none bg-amber-500 text-white text-xl font-extralight transition duration-300 hover:bg-amber-500 hover:scale-105 "
                                ...
                                <div className="sm:flex sm:items-start">
                                    <div className="mr-6 mt-3 w-full text-center sm:ml-4 sm:mt-0 sm:text-left">
                                        <form className="my-0 flex flex-col gap-5">
                                            <ModalInput value={formData.name} setValue={setFormData} placeHolder="name"
                                                        reauired={true}/>
                                            <ModalInput value={formData.address} setValue={setFormData}
                                                        placeHolder="address"
                                                        reauired={true}/>
                                            <ModalInput value={formData.city} setValue={setFormData} placeHolder="city"
                                                        reauired={false}/>
                                        </form>
                                    </div>
                                </div>
